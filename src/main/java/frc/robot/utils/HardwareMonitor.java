...
public class HardwareMonitor implements AutoCloseable, Sendable {
	...
	public HardwareMonitor() {
		...
		this.notifier.setName(this.getClass().getSimpleName());
	...
	private static class DeviceCANSparkBase extends Device {
		private SparkBase device;

		DeviceCANSparkBase(SparkBase device) {
			this.device = device;
		}

		@Override
		String getName() {
			if (device instanceof SparkFlex) {
		...
		List<String> getErrors() {
			List<String> errors = new ArrayList<String>();
			var faults = device.getFaults().rawBits;

			if (faults != 0) {
				for (int bit = 0; bit < 15; bit++) {
					if (((faults >> bit) & 1) != 0) {
						errors.add(device.getFaults().toString());
					}
				}
			}
	...
	/** Add a new device to the hardware monitor. */
	public void registerDevice(Subsystem subsystem, SparkBase device) {
	...
	public static class DevicePowerDistribution extends Device {
		...
		public DevicePowerDistribution withChannelNames(String... names) {
		...
		List<String> getErrors() {
			...
			for (int channel = 0; channel < 24; channel++) {
				if (channel < this.channelNames.length && this.channelNames[channel] == null) {
					// Ignore PDH channels that are supposed to be unplugged.
					continue;
				}

				if (faults.getBreakerFault(channel)) {
					if (channel < this.channelNames.length) {
						errors.add("Breaker #" + channel + "(" + this.channelNames[channel] + ") tripped");
					} else {
	...
	/** Add a new device to the hardware monitor. */
	public DevicePowerDistribution registerDevice(Subsystem subsystem, PowerDistribution device) {
		var result = new DevicePowerDistribution(device);
		this.registerDevice(subsystem, result);
		return result;
	}

	/** A `Device` specialized for an Xbox Controller. */
	public static class DeviceGenericHID extends Device {
		private GenericHID device;

		DeviceGenericHID(GenericHID device) {
			this.device = device;
		}

		@Override
		String getName() {
			return "Gamepad #" + device.getPort();
		}
	...
	/** Add a new device to the hardware monitor. */
	public DeviceGenericHID registerDevice(Subsystem subsystem, GenericHID device) {
		var result = new DeviceGenericHID(device);
		this.registerDevice(subsystem, result);
		return result;
	}

	/** Add a new device to the hardware monitor. */
	public DeviceGenericHID registerDevice(Subsystem subsystem, CommandGenericHID device) {
		var result = new DeviceGenericHID(device.getHID());
		this.registerDevice(subsystem, result);
		return result;
	}

	/** A `Device` specialized for a PhotonVision Camera. */
	public static class DevicePhoton extends Device {
		private PhotonCamera device;

		DevicePhoton(PhotonCamera device) {
			this.device = device;
		}
		...
		List<String> getErrors() {
			...
			if (!device.isConnected()) {
				errors.add("Disconnected");
			} else if (device.getDistCoeffs().isEmpty()) {
				errors.add("Uncalibrated");
			}
			return errors;
		}
	}
	...
	/** Add a new device to the hardware monitor. */
	public DevicePhoton registerDevice(Subsystem subsystem, PhotonCamera device) {
		var result = new DevicePhoton(device);
		this.registerDevice(subsystem, result);
		return result;
	}
	...
	public void initSendable(SendableBuilder builder) {
		for (Integer i = 0; i < 10; i++) {
			...
			builder.addStringProperty(i.toString(), () -> this.getErrorLine(errorIndex), null);
		}
	}
}
