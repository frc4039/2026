...
public class FinalTowerAlignCommand extends DeferredCommand {
	public FinalTowerAlignCommand(DriveSubsystem driveSubsystem, VisionSubsystem visionSubsystem) {
	...
	public static Command getPathCommand(DriveSubsystem driveSubsystem, VisionSubsystem visionSubsystem) {
		Pose2d Target = VisionConstants.KFinalRedTower;
		double endX;
		double endY;
		...
		endX = Target.getX();
		// + finalXOffset * Math.cos(Target.getRotation().getRadians()) - finalYOffset *
		// Math.sin(Target.getRotation().getRadians());
		endY = Target.getY();
		// + finalXOffset * Math.sin(Target.getRotation().getRadians()) + finalYOffset *
		// Math.cos(Target.getRotation().getRadians());
		...
		// Only drive to path when total distance to target is greater than .02m
		if ((x * x + y * y) > (0.02 * 0.02)) {
			...
			PathConstraints constraints = new PathConstraints(0.5, 1.0, 2 * Math.PI, 4 * Math.PI);
			PathPlannerPath path = new PathPlannerPath(
					waypoints,
					constraints,
					null,
					new GoalEndState(0.0, Rotation2d.fromDegrees((Target.getRotation().getDegrees()))));
